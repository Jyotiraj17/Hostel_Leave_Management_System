...
function renderRoomsTable(container, rooms) {
  if (rooms.length === 0) {
    container.innerHTML = `
      <div class="empty-state" style="padding: 3rem 1rem;">
        ...
        <p style="margin-top:0.35rem; color:var(--text-muted);">Click "Add New Room" above to register a room.</p>
      </div>
    `;
    if (window.lucide) lucide.createIcons();
    return;
            ...
            <tr>
              ...
              <td>
                <div style="display: flex; align-items: center; justify-content: flex-end; gap: 8px; white-space: nowrap;">
                  ...
                  <button class="btn-icon btn-icon-edit edit-room-btn" data-id="${r._id}" title="Edit Room" aria-label="Edit Room">
                    <i data-lucide="edit"></i>
                  </button>
                  <button class="btn-icon btn-icon-delete delete-room-btn" data-id="${r._id}" data-number="${r.roomNumber}" title="Delete Room" aria-label="Delete Room">
  ...
  // View Occupants Event
  container.querySelectorAll('.view-occupants-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const room = allRoomsData.find(r => r._id === btn.dataset.id);
      if (!room) return;
  ...
  container.querySelectorAll('.edit-room-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.getElementById('edit-room-id').value = room._id;
      document.getElementById('edit-r-capacity').value = room.capacity;
      document.getElementById('edit-r-floor').value = room.floor;
      document.getElementById('edit-r-status').value = room.status;
...
function showOccupantsModal(room) {
  let modal = document.getElementById('occupants-modal');
  if (!modal) {
    const div = document.createElement('div');
    div.id = 'occupants-modal';
    div.className = 'modal-backdrop';
    div.innerHTML = `
      <div class="modal-content">
        <div class="modal-header">
          <div class="modal-title">Room Occupant Details</div>
          <button class="modal-close" onclick="closeModal('occupants-modal')">&times;</button>
        </div>
        <div class="modal-body" id="occupants-modal-body"></div>
        <div class="modal-footer">
          <button class="btn btn-secondary" onclick="closeModal('occupants-modal')">Close</button>
        </div>
      </div>
    `;
  ...
  const modalBody = document.getElementById('occupants-modal-body');
...
function initAssignModalHandlers() {
  ...
  assignForm.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await api.post('/admin/rooms/assign', { studentId, roomId });
      if (res.success) {
...
function initRoomFormHandlers() {
  const addForm = document.getElementById('add-room-form');
  if (addForm) {
    addForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const roomNumber = document.getElementById('add-r-number').value.trim();
      const hostel = document.getElementById('add-r-hostel').value.trim();
      const block = document.getElementById('add-r-block').value.trim();
      const floor = Number(document.getElementById('add-r-floor').value);
      const capacity = Number(document.getElementById('add-r-capacity').value);

      if (!roomNumber || !hostel || !block || capacity < 1) {
      ...
      try {
        const res = await api.post('/admin/rooms', { roomNumber, hostel, block, floor, capacity });
        if (res.success) {
          ...
          toast.error(res.message || 'Failed to add room.');
        }
      } catch (err) {
  ...
  if (editForm) {
    editForm.addEventListener('submit', async (e) => {
      ...
      const capacity = Number(document.getElementById('edit-r-capacity').value);
      const floor = Number(document.getElementById('edit-r-floor').value);
      const status = document.getElementById('edit-r-status').value;

      const updateBtn = document.getElementById('update-room-btn');
      updateBtn.disabled = true;

      try {
        const res = await api.put(`/admin/rooms/${id}`, { capacity, floor, status });
        if (res.success) {
