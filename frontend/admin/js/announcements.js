...
function renderAnnouncementsList(container, list) {
  ...
  container.querySelectorAll('.delete-ann-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        const res = await api.delete(`/admin/announcements/${annId}`);
        if (res.success) {
...
function initAnnouncementForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      let res;
      if (annId) {
        res = await api.put(`/admin/announcements/${annId}`, { title, description, priority, expiryDate });
      } else {
        res = await api.post('/admin/announcements', { title, description, priority, expiryDate });
      }
