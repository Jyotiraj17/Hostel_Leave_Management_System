...
async function loadLeaveRequests() {
  ...
  try {
    const res = await api.get('/warden/leaves');
    if (!res.success) {
      container.innerHTML = `<div class="empty-state">Unable to load leave requests.</div>`;
      return;
    }
...
function renderLeavesTable(container, leaves) {
              ...
              <tr>
                ...
                <td>
                  <div style="display: flex; align-items: center; justify-content: flex-end; gap: 8px; white-space: nowrap;">
                    ...
                    ${l.status === 'Pending' ? `
                      <button class="btn-icon btn-icon-edit approve-leave-btn" data-id="${l._id}" title="Approve Leave" style="color:var(--success); background-color:var(--success-bg); border-color:var(--success);">
                        <i data-lucide="check"></i>
                      </button>
                      <button class="btn-icon btn-icon-delete reject-leave-btn" data-id="${l._id}" title="Reject Leave">
                        <i data-lucide="x"></i>
                      </button>
                    ` : ''}
                  </div>
  ...
  container.querySelectorAll('.view-leave-btn').forEach(btn => {
    btn.addEventListener('click', () => openLeaveViewModal(btn.dataset.id));
  });
...
async function openLeaveViewModal(leaveId) {
  try {
    const res = await api.get(`/warden/leaves/${leaveId}`);
    if (!res.success || !res.leave) return;
...
function openApproveModal(leaveId) {
  ...
  document.getElementById('action-modal-title').textContent = 'Approve Leave Request';
  document.getElementById('action-remarks').value = 'Approved by Warden';
...
function openRejectModal(leaveId) {
  ...
  document.getElementById('action-modal-title').textContent = 'Reject Leave Request';
